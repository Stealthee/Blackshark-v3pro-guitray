...
THX Spatial Audio needs a one-time extra setup step (a PipeWire HRTF convolver, not
Razer's proprietary Windows DSP) — see [THX_SETUP.md](THX_SETUP.md).
...
rpmbuild -bb --define "_sourcedir $PWD/.." packaging/rpm/lynapse.spec
sudo dnf install ~/rpmbuild/RPMS/noarch/lynapse-*.rpm
```
...
This installs `data/lynapse-autostart.desktop` to
`~/.config/autostart/lynapse.desktop`. To remove it again:

```sh
rm ~/.config/autostart/lynapse.desktop
```

## Usage

Launch from your application menu (entry: **Lynapse**) or run:
...
Per-slot custom EQ values persist in `~/.config/lynapse.json`.
...
- `packaging/rpm/lynapse.spec` (`Version:` + add a `%changelog` entry)
